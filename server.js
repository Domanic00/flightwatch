...
app.get("/api/profile",async(q,r)=>r.json(await db.getUserProfile(q.session?.user)));app.put("/api/profile",async(q,r)=>r.json(await db.saveUserProfile(q.session?.user,q.body||{})));
