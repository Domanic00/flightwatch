...
const dataDir=path.join(__dirname,"..",".data"), dataFile=path.join(dataDir,"anywhere-with-you.json");
const blank=()=>({
...
CREATE TABLE IF NOT EXISTS super_todos(
  id bigserial primary key,
  title text not null,
...
CREATE INDEX IF NOT EXISTS super_todos_status_idx ON super_todos(status,created_at desc);
...
CREATE INDEX IF NOT EXISTS admin_email_log_created_idx ON admin_email_log(created_at desc);
...
async function createUserReport(user,title,description){
  ...
  if(description.length>4000)throw new Error("Description is too long");
  const reporterKey=ownerKey(user),email=user?.email||null;
  if(!usePostgres){
    let d=read();d.userReports=d.userReports||[];
    const row={id:Date.now(),reporterKey,reporterEmail:email,title,description,status:"open",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
    d.userReports.unshift(row);write(d);return row
  }
  const {rows}=await pool.query(`insert into user_reports(reporter_key,reporter_email,title,description) values($1,$2,$3,$4)
    returning id,reporter_key as "reporterKey",reporter_email as "reporterEmail",title,description,status,created_at as "createdAt",updated_at as "updatedAt"`,
    [reporterKey,email,title,description]);
  return rows[0];
}
async function getUserReports(limit=200){
  limit=Math.min(500,Math.max(1,Number(limit)||200));
  if(!usePostgres)return (read().userReports||[]).slice(0,limit);
  return (await pool.query(`select id,reporter_key as "reporterKey",reporter_email as "reporterEmail",title,description,status,assigned_to as "assignedTo",admin_note as "adminNote",created_at as "createdAt",updated_at as "updatedAt" from user_reports order by case when status='open' then 0 when status='acknowledged' then 1 else 2 end,created_at desc limit $1`,[limit])).rows;
}
async function updateUserReport(id,patch,user){
  const allowed=["open","acknowledged","resolved"];
  const status=patch?.status&&allowed.includes(String(patch.status))?String(patch.status):null;
  const note=patch?.adminNote!==undefined?String(patch.adminNote||"").slice(0,2000):null;
  const assignee=patch?.assignedTo!==undefined?String(patch.assignedTo||"").slice(0,200):null;
  if(!usePostgres)throw new Error("Report updates require PostgreSQL");
  const old=(await pool.query("select * from user_reports where id=$1",[id])).rows[0];if(!old)throw new Error("Report not found");
  const {rows}=await pool.query(`update user_reports set
      status=coalesce($2,status),
      admin_note=case when $3::text is null then admin_note else $3 end,
      assigned_to=case when $4::text is null then assigned_to else $4 end,
      updated_at=now()
    where id=$1
    returning id,reporter_key as "reporterKey",reporter_email as "reporterEmail",title,description,status,assigned_to as "assignedTo",admin_note as "adminNote",created_at as "createdAt",updated_at as "updatedAt"`,
    [id,status,note,assignee]);
  await audit("user_report_updated",`Report #${id} → ${rows[0].status}`,user?.email||"admin");
  return rows[0];
}


async function getSuperTodos(){
  if(!usePostgres)return [];
  return (await pool.query(`select id,title,remarks,status,created_by as "createdBy",created_at as "createdAt",updated_at as "updatedAt"
    from super_todos order by case when status='open' then 0 else 1 end, created_at desc`)).rows;
}
async function createSuperTodo(title,remarks,user){
  title=String(title||"").trim();remarks=String(remarks||"").trim();
  if(title.length<2)throw new Error("To-do title is required");
  if(title.length>180)throw new Error("To-do title is too long");
  if(remarks.length>4000)throw new Error("Remarks are too long");
  if(!usePostgres)throw new Error("To-do list requires PostgreSQL");
  const {rows}=await pool.query(`insert into super_todos(title,remarks,created_by) values($1,$2,$3)
    returning id,title,remarks,status,created_by as "createdBy",created_at as "createdAt",updated_at as "updatedAt"`,
    [title,remarks,user?.email||"super_admin"]);
  await audit("super_todo_created",`To-do #${rows[0].id}: ${title}`,user?.email||"super_admin");
  return rows[0];
}
async function updateSuperTodo(id,patch,user){
  const status=patch?.status&&["open","done"].includes(String(patch.status))?String(patch.status):null;
  const remarks=patch?.remarks!==undefined?String(patch.remarks||"").slice(0,4000):null;
  const title=patch?.title!==undefined?String(patch.title||"").trim().slice(0,180):null;
  if(!usePostgres)throw new Error("To-do list requires PostgreSQL");
  const {rows}=await pool.query(`update super_todos set
    title=case when $2::text is null then title else $2 end,
    remarks=case when $3::text is null then remarks else $3 end,
    status=coalesce($4,status),
    updated_at=now()
    where id=$1 returning id,title,remarks,status,created_by as "createdBy",created_at as "createdAt",updated_at as "updatedAt"`,
    [id,title,remarks,status]);
  if(!rows.length)throw new Error("To-do not found");
  await audit("super_todo_updated",`To-do #${id} → ${rows[0].status}`,user?.email||"super_admin");
  return rows[0];
}
async function deleteSuperTodo(id,user){
  if(!usePostgres)throw new Error("To-do list requires PostgreSQL");
  await pool.query("delete from super_todos where id=$1",[id]);
  await audit("super_todo_deleted",`To-do #${id}`,user?.email||"super_admin");
}


async function listActiveRecipientEmails(scope="all",selected=[]){
  if(!usePostgres)return [];
  let rows=[];
  if(scope==="selected"){
    const emails=(selected||[]).map(x=>String(x||"").trim().toLowerCase()).filter(Boolean);
    if(!emails.length)return [];
    rows=(await pool.query(`select email from beta_users where enabled=true and coalesce(status,'active')='active' and lower(email)=any($1::text[])`,[emails])).rows;
  }else{
    rows=(await pool.query(`select email from beta_users where enabled=true and coalesce(status,'active')='active' order by email`)).rows;
  }
  return rows.map(r=>r.email).filter(Boolean);
}
async function logAdminEmail(senderEmail,scope,count,subject,bodyPreview,summary){
  if(!usePostgres)return;
  await pool.query(`insert into admin_email_log(sender_email,recipient_scope,recipient_count,subject,body_preview,delivery_summary)
    values($1,$2,$3,$4,$5,$6::jsonb)`,
    [senderEmail||"admin",scope,count,String(subject||"").slice(0,250),String(bodyPreview||"").slice(0,500),JSON.stringify(summary||{})]);
}

module.exports={init,listActiveRecipientEmails,logAdminEmail,getSuperTodos,createSuperTodo,updateSuperTodo,deleteSuperTodo,createUserReport,getUserReports,updateUserReport,bootstrapSuperAdmin,securityEvent,listManagedUsers,setManagedUserRole,setManagedUserStatus,getSecurityEvents,getGroups,createGroup,inviteToGroup,getGroupInvites,acceptGroupInvite,getUserProfile,saveUserProfile,findBetaUserByEmail,setBetaPassword,markBetaLogin,logError,getErrors,setErrorStatus,audit,getAudit,getBetaUsers,addBetaUser,removeBetaUser,usePostgres,getProfile,saveProfile,getTracked,addTracked,removeTracked,getWatchlists,addWatchlist,removeWatchlist,saveSnapshots,history,routeStats,alertExists,createAlert,updateAlertDelivery,getAlerts,setMeta,status};
