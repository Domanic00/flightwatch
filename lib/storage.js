...
async function getUserProfile(user){
  if(!user || user.role==="admin") return getProfile();
  if(!usePostgres){
...
async function saveUserProfile(user,p){
  if(!user || user.role==="admin") return saveProfile(p);
...
const ownerKey=u=>u?.role==="admin"?"admin":String(u?.id||"anonymous");
...
module.exports={init,getGroups,createGroup,inviteToGroup,getGroupInvites,acceptGroupInvite,getUserProfile,saveUserProfile,findBetaUserByEmail,setBetaPassword,markBetaLogin,logError,getErrors,setErrorStatus,audit,getAudit,getBetaUsers,addBetaUser,removeBetaUser,usePostgres,getProfile,saveProfile,getTracked,addTracked,removeTracked,getWatchlists,addWatchlist,removeWatchlist,saveSnapshots,history,routeStats,alertExists,createAlert,updateAlertDelivery,getAlerts,setMeta,status};
